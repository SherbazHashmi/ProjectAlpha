...
public class DifficultyBackgroundChange : MonoBehaviour
{
	...
	void Start ()
    {
        //backGround = GetComponentInChildren<Image>();                                           //Gets the sprite renderer component from the Child object
        confirmPoppup.SetActive(false);                                                         //disables confirm popup
	}

    public void EasyPressed()
    {
        backGround.sprite = easyImage;                                                         //Changes the images to easyImage
        confirmPoppup.SetActive(true);                                                          //enables confirm popup
    }

    public void NormalPressed()
    {
        backGround.sprite = normalImage;                                                       //Changes the images to normalImage
        confirmPoppup.SetActive(true);                                                          //enables confirm popup
    }

    public void HardPressed()
    {
        backGround.sprite = hardImage;                                                         //Changes the images to hardImage
        confirmPoppup.SetActive(true);                                                          //enables confirm popup
    }

    public void ConfirmPressed()
    {
        SceneManager.LoadScene("LevelOne");                                                    //Loads scene indicated with ""
    }
    ...
    {
        SceneManager.LoadScene("MainMenuScene");                                                    //Loads scene indicated with ""
    }
}
